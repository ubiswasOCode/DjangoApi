...
def signup(request):
    if request.method=="POST":
        ...
        r = requests.post('http://127.0.0.1:8000/api/createuser', data=request.POST)
...
def CreatePro(request):
    ...
    if request.method== "POST":
        ...
        response=requests.post('http://127.0.0.1:8000/api/createproduct/', data=request.POST)
        print(response)
        print(request)
...
def DelPro(request):
    if request.method == "POST":
        id = request.POST["id"]
        response=requests.delete(f'http://127.0.0.1:8000/api/deleteproduct/{id}/')
        print(response)
...
def CreateCate(request):
    ...
    if request.method== "POST":
        print(request.POST,"Data________")
        response=requests.post('http://127.0.0.1:8000/api/createcategory/', data=request.POST)
        print(response)
